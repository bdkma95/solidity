 ...
 *
 * @dev The Token is an ERC20 Token
 * @dev https://github.com/ethereum/EIPs/issues/20
 */
contract Token is ERC20Interface, Ownable, SafeMath {
...
contract Voting {
    ...
    // time in epoche unix time seconds
    ...
    // we need to search and iterate through voters..
    address[] public voterArr;
    mapping(address => Voter) voterMap;

    // After voting
    uint256[] summedVotes;
    uint256 public summedUpIndex = 0;

    constructor(address _tokenAddress, string _proposal, uint8 _numberOfOptions, uint256 _startTime, uint256 _endTime) public {
        require(_tokenAddress != address(0));
        require(_numberOfOptions > 1);
        require(_startTime > block.timestamp);
        require(_endTime > block.timestamp);
        require(_endTime > _startTime);
        
        token = Token(_tokenAddress);
        proposal = _proposal; //e.g. would you like to have a dividende. Option 0: No, Option 1: Yes
        numberOfOptions = _numberOfOptions; //e.g. 2. depends on the proposal question
        
        startTime = _startTime;
        endTime = _endTime;
        
        summedVotes = new uint256[](_numberOfOptions);
    }

    function vote(uint8 _proposal) public returns (bool) {
        require(_proposal < numberOfOptions);
        require(voterMap[msg.sender].hasVoted == false);
       // require(token.balanceOf(msg.sender) > 0);
       // require(token.isLocked(msg.sender) == false);
        require(startTime >= block.timestamp);

        voterMap[msg.sender].hasVoted = true;
        voterMap[msg.sender].option = _proposal;
    ...
    /**
     * @dev This method has to be called quickly (directly) after the "end". As often until leftVotesToSummUp() returns 0.
     */
    function sumUp(uint256 _length) public returns (bool) {
        require(_length > 0);
        require(summedUpIndex < voterArr.length);

        uint256 toIndex = summedUpIndex + _length;
        require(toIndex <= voterArr.length);

        for(uint256 i = summedUpIndex; i<toIndex; i++) {
            address voterAddr = voterArr[i];
            uint256 weight = 100; 
            uint8 _proposal = voterMap[voterAddr].option;
            summedVotes[_proposal] = summedVotes[_proposal] + weight;
        }

        summedUpIndex = toIndex;
        return true;
    }

    function totalVoters() public view returns (uint256) {
        return voterArr.length;
    }

    function leftVotesToSummUp() public view returns (uint256) {
        //require now > end

        return voterArr.length - summedUpIndex;
    }

    function hasResult() public view returns (bool) {
        return summedUpIndex == (voterArr.length - 1);
    }

    function getVotes(uint8 _proposal) public view returns (uint256) {
        require(hasResult());

        return summedVotes[_proposal];
    }
    ...
    function getWinningOption() public view returns (uint8) {
        ...
        for(uint8 i = 0; i<numberOfOptions; i++) {
            if(summedVotes[i] > highestRate) {
                highestRate = summedVotes[i];
                winnerOption = i;
            }
        }
